...
定义 **效果组合（Composition）**：可执行的组合树，连接卡面文本、Initiation resolve 与 L0 原子 / Registration。
...
几乎 **全部** 效果 resolve 只做两件事之一：

| 类型 | 改什么 | 组合节点 |
|---|---|---|
| **状态变更** | `GameStateStore`（卡位、token…） | L0 **Atom** |
| **规则变更** | `RegistrationStore`（挂/摘 Buff） | L2 **Register** / **Unregister** |

Cancel / Replacement、Draw、DealDamage 等 **不是** 第三类机制：前者是 L2 组合节点；后者是 **L3 宏** 展开为 L0/L1/L2。
...
| 层 | kind | 说明 |
|---|---|---|
| L0 | `Atom` | [07-effect-primitives](07-effect-primitives.md) 五原子 |
...
- **Register**：`RegistrationStore` 中 **成功插入一条 Registration** → 该节点 **CREATED**
- **Atom**：`StateMutator` 在 fork 上 **执行成功** → **CREATED**
