...
| `[fast]` | **ArkhamDB 文本**：Core 玩家牌用此标记 **免费触发**（闪电图标）；引擎 `segment.kind=fast` → `register_as:free`。**另**：规则书 **Fast.** 关键词改的是 **打出** 的窗口/花费（种类仍是打出），见 [06-ability-initiation §4.2](../design/06-ability-initiation.md#42-打出与触发对称fast打出-不是-能力)；不经本触发分段。 |
